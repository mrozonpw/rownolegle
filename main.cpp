...
)
{
    ...
    for (int threads : {2, 4})
    {
        ...
        auto result_par = perform_sequential_algorithm(func, x0, n, a, b);
...
int main()
{
    ...
    run_test(
        "QUADRATIC FUNCTION",
        calc_quadratic_function,
        make_quadratic_x0,
        [](const std::vector<double>& x, uint32_t) { return l2_norm(x); },
        n,
        -5,
        5
    );
    ...
    run_test(
        ...
        calc_powell_singular_function,
        make_powell_x0,
        l2_norm_distance_to_powell_min,
        n,
        -4,
        4
    );
